...
private static BuyDBBean instance = new BuyDBBean();
    ...
    public void insertBuy( List<CartDataBean> lists,
        ...
        try {
            ...
            for(int i=0; i<lists.size();i++){
            	...
            	sql = "insert into buy (buy_id,buyer,product_id,product_name,buy_price,buy_count,";
                sql += "product_image,buy_date,account,deliveryName,deliveryTel,deliveryAddress)";
                sql += " values (?,?,?,?,?,?,?,?,?,?,?,?)";
                ...
                pstmt = conn.prepareStatement(
                		"select product_count from product where product_id=?");
    ...
    public List<BuyDataBean> getBuys(int start, int end)
            throws Exception {
       Connection conn = null;
       PreparedStatement pstmt = null;
       ResultSet rs = null;
       List<BuyDataBean> buyList1=null;
       try {
           conn = getConnection();
           
           pstmt = conn.prepareStatement(
           	"select * from member order by reg_date limit ?,?");
           pstmt.setInt(1, start-1);
			pstmt.setInt(2, end);
           rs = pstmt.executeQuery();

           if (rs.next()) {
               buyList1 = new ArrayList<BuyDataBean>(end);
               do{
