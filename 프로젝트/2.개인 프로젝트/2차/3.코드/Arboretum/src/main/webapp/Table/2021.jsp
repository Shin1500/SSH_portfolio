...
<%
  ...
  try {
    ...
    String sql = "SELECT SUM(Count) AS total_count FROM tourist WHERE Year = '2021년'";
    pstmt = conn.prepareStatement(sql);
    rs = pstmt.executeQuery();
    
    if (rs.next()) {
      total_2021 = rs.getInt("total_count");
    }
  } catch (Exception e) {
    e.printStackTrace();
  }
%>
<h2>2021년 총 관람객 수: <%= total_2021 %>명</h2>
...
<% //DB연결
  ...
  try{
	 ...
	 rs=pstmt.executeQuery();
	 if(rs.next()){
		 month_2021 = new String[rs.getInt(1)];
		 count_2021 = new int[rs.getInt(1)];
	 }
	 
	 sql= "select * from tourist where Year = '2021년'";
	 pstmt=conn.prepareStatement(sql);
	 rs=pstmt.executeQuery();
	 int i=0;
	 while(rs.next()){
...
<div id="chart-container" style="display: none; vertical-align: top;">
    <!-- 차트를 포함한 2021.jsp 파일을 불러와서 표시 -->
    <iframe src="../Chart/2021.jsp" width="900" height="500" frameborder="0"></iframe>
</div>
<!-- 홈으로 버튼 -->
<div id="home-btn">
